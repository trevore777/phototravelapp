...
function isPreviewSafeExtension(ext: string) {
...
export async function POST(req: NextRequest) {
  try {
    ...
    for (let i = 0; i < files.length; i++) {
      ...
      if (isPreviewSafeExtension(extension)) {
        try {
          await sharp(bytes)
            .rotate()
      ...
      let meta: Awaited<ReturnType<typeof parsePhotoMeta>>;
      try {
        meta = await parsePhotoMeta(bytes);
      } catch {
        meta = {};
      }
      ...
      await db.photo.create({
        data: {
          ...
          orientation:
            typeof meta.orientation === "number" && Number.isInteger(meta.orientation)
              ? meta.orientation
              : null,
    ...
    return NextResponse.json(
      {
        error: "Upload route failed on the server. Check terminal output for the underlying cause."
      },
      { status: 500 }
    );
  }
